...
class Decorate extends _Events.Events {
  ...
  _injectDecorated(registry, fromEmpty, blockHash) {
    const decoratedMeta = (0, _types.expandMetadata)(registry.registry, registry.metadata); // clear the decoration, we are redoing it here
    ...
    (0, _augmentObject.augmentObject)('consts', decoratedMeta.consts, registry.decoration.consts, fromEmpty);
    (0, _augmentObject.augmentObject)('errors', decoratedMeta.errors, registry.decoration.errors, fromEmpty);
    (0, _augmentObject.augmentObject)('events', decoratedMeta.events, registry.decoration.events, fromEmpty);
    ...
    (0, _augmentObject.augmentObject)('query', storage, registry.decoration.query, fromEmpty);
    return {
  ...
  _decorateStorageCall(creator, decorateMethod) {
    ...
    return decorateMethod((...args) => {
      return this.hasSubscriptions ? this._rpcCore.state.subscribeStorage([(0, _validate.extractStorageArgs)(creator, args)]).pipe((0, _rxjs.map)(([data]) => data) // extract first/only result from list
  ...
  _decorateStorageRange(decorated, args, range) {
    const outputType = (0, _types.unwrapStorageType)(decorated.creator.meta.type, decorated.creator.meta.modifier.isOptional);
