  ...
  ***************************************************************************** */
  ...
  class RpcCore {
    ...
    _formatInputs(registry, blockHash, def, inputs) {
      ...
      return inputs.map((input, index) => types.createTypeUnsafe(registry, def.params[index].type, [input], {
        blockHash
      }));
    }
    ...
    _formatOutput(registry, blockHash, method, rpc, params, result) {
      ...
      return types.createTypeUnsafe(registry, rpc.type, [result], {
        blockHash
      });
    }
    ...
    _newType(registry, blockHash, key, input, isEmpty, entryIndex = -1) {
      ...
      try {
        if (meta.modifier.isOptional) {
          ...
          if (!isEmpty) {
            inner = types.createTypeUnsafe(registry, type, [input], {
              blockHash,
              isPedantic: true
            });
          }

          const option = new types.Option(registry, types.createClass(registry, type), inner);
        ...
        return types.createTypeUnsafe(registry, type, [isEmpty ? meta.fallback ? util.hexToU8a(meta.fallback.toHex()) : undefined : input], {
          blockHash,
          isPedantic: true
        });
      } catch (error) {
  ...
  function calcBalances(api, [accountId, [accountNonce, balances]]) {
    ...
    util.assert(primary, 'No balances retrieved for account');
  ...
  function queryBalancesAccount(api, accountId, modules = ['balances']) {
    ...
    return util.isFunction(api.query.system.account) ? api.queryMulti([[api.query.system.account, accountId], ...balances]).pipe(map(([{
      nonce
  ...
  function account$1(instanceId, api) {
    ...
    return memo(instanceId, address => api.derive.accounts.accountId(address).pipe(switchMap(accountId => {
      ...
      return accountId ? combineLatest([of(accountId), balanceInstances ? queryBalancesAccount(api, accountId, balanceInstances) : util.isFunction((_api$query$system = api.query.system) === null || _api$query$system === void 0 ? void 0 : _api$query$system.account) ? querySystemAccount(api, accountId) : util.isFunction((_api$query$balances = api.query.balances) === null || _api$query$balances === void 0 ? void 0 : _api$query$balances.account) ? queryBalancesAccount(api, accountId) : util.isFunction((_api$query$balances2 = api.query.balances) === null || _api$query$balances2 === void 0 ? void 0 : _api$query$balances2.freeBalance) ? queryBalancesFree(api, accountId) : util.isFunction((_api$query$system2 = api.query.system) === null || _api$query$system2 === void 0 ? void 0 : _api$query$system2.accountNonce) ? querySystemNonce(api, accountId) : of(empty)]) : of([api.registry.createType('AccountId'), empty]);
  ...
  function doDoubleMap(creator, args) {
    ...
    util.assert(args.length === 2, () => `${sig(creator, [key1, key2])} is a double map, requiring 2 arguments, ${args.length} found`); // pass as tuple
  ...
  function doMap(creator, args) {
    ...
    util.assert(args.length === 1, () => `${sig(creator, [key])} is a map, requiring 1 argument, ${args.length} found`); // expand
  ...
  function doNMap(creator, args) {
    ...
    util.assert(args.length === keyVec.length, () => `${sig(creator, keyVec)} is a multi map, requiring ${keyVec.length} arguments, ${args.length} found`); // pass as tuple
  ...
  function extractStorageArgs(creator, _args) {
    ...
    util.assert(args.length === 0, () => `${sig(creator, [])} does not take any arguments, ${args.length} found`); // no args
  ...
  class Decorate extends Events {
    ...
    _injectDecorated(registry, fromEmpty, blockHash) {
      const decoratedMeta = types.expandMetadata(registry.registry, registry.metadata); // clear the decoration, we are redoing it here
      ...
      const storage = blockHash ? this._decorateStorageAt(decoratedMeta, this._decorateMethod, blockHash) : this._decorateStorage(decoratedMeta, this._decorateMethod);
    ...
    _injectMetadata(registry, fromEmpty) {
      const {
        decoratedApi,
        decoratedMeta
      } = this._injectDecorated(registry, fromEmpty);
    ...
    _decorateStorageRange(decorated, args, range) {
      const outputType = types.unwrapStorageType(decorated.creator.meta.type, decorated.creator.meta.modifier.isOptional);
  ...
  class Init extends Decorate {
    ...
    async at(blockHash) {
      ...
      return registry.decoration ? registry.decoration : this._injectDecorated(registry, true, u8aHash).decoratedApi;
    }
    /**
    ...
    async getBlockRegistry(blockHash) {
      ...
      } : await firstValueFrom(this._rpcCore.state.getRuntimeVersion.json(header.parentHash))); // check for pre-existing registries
