const polkadotKeyring = (function (exports, util, utilCrypto, packageInfo$1) {
  'use strict';
  ...
  function createPair({
    ...
    return {
      ...
      encryptMessage: (message, recipientPublicKey, _nonce) => {
        util.assert(!isLocked(secretKey), 'Cannot encrypt with a locked key pair');
        util.assert(!['ecdsa', 'ethereum'].includes(type), 'Secp256k1 not supported yet');
        const {
          nonce,
          sealed
        } = utilCrypto.naclSeal(util.u8aToU8a(message), utilCrypto.convertSecretKeyToCurve25519(secretKey), utilCrypto.convertPublicKeyToCurve25519(util.u8aToU8a(recipientPublicKey)), _nonce);
      ...
      verify: (message, signature, _signerPublic) => {
        return utilCrypto.signatureVerify(message, signature, TYPE_ADDRESS[type](util.u8aToU8a(_signerPublic))).isValid;
      },
      ...
      vrfVerify: (message, vrfResult, _signerPublic, context, extra) => {
        const signerPublic = TYPE_ADDRESS[type](util.u8aToU8a(_signerPublic));

        if (type === 'sr25519') {
          return utilCrypto.schnorrkelVrfVerify(message, vrfResult, publicKey, context, extra);
        }

        const result = utilCrypto.signatureVerify(message, util.u8aConcat(TYPE_PREFIX[type], vrfResult.subarray(32)), signerPublic);
  ...
  // Copyright 2017-2021 @polkadot/keyring authors & contributors
  util.detectPackage(packageInfo, typeof __dirname !== 'undefined' && __dirname, [util.packageInfo, packageInfo$1.packageInfo]);
...
}({}, polkadotUtil, polkadotUtilCrypto, packageInfo$1));
