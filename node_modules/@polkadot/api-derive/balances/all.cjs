...
function queryCurrent(api, accountId, balanceInstances = ['balances']) {
  ...
  : lockQueries.length ? api.queryMulti(lockQueries).pipe((0, _rxjs.map)(locks => [api.registry.createType('Option<VestingInfo>'), ...locks])) : (0, _rxjs.of)([api.registry.createType('Option<VestingInfo>')])).pipe((0, _rxjs.map)(([optVesting, ...locks]) => {
    let offset = -1;
